...
class Character extends Database {
    ...
    public function update_character($char_fname, $char_lname, $char_id) {
        $db = $this->connectDatabase();
        $q = "UPDATE `playable_characters` SET `char_fname`=:char_fname,`char_lname`=:char_lname WHERE `playable_characters`.`char_id`=:char_id;";
    ...
    public function delete_character($char_id) {
        $db = $this->connectDatabase();
        $q = "DELETE FROM `playable_characters` WHERE `playable_characters`.`char_id`=:char_id;";
        $del_char_q = $db->prepare($q);
        $del_char_q->bindValue(':char_id', $char_id, PDO::PARAM_INT);
        $del_char_q->execute();
    }
}